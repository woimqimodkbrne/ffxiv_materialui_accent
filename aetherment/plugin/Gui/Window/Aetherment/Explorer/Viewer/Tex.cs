...
public class Tex: Viewer {
	...
	private unsafe void ReloadPreview() {
		var d = GetPreview(tex, (ushort)preview.Width, (ushort)preview.Height, (ushort)miplevel, (ushort)depth);
		if(!d.IsOk(out previewData)) {
			ShowError(d.Error());
			return;
		}
	...
	protected override unsafe void DrawViewer() {
		...
		if(header.format != Format.A8 && header.format != Format.L8) {
		...
		// TODO: fancier sliders, split width to fill
		if(header.mip_levels > 1) {
		ImGui.SetNextItemWidth(200);
			changed |= ImGui.SliderInt("Mip Level", ref miplevel, 0, header.mip_levels - 1);
			ImGui.SameLine();
		}
		miplevel = Math.Min(miplevel, tex->header.mip_levels - 1);
		
		var maxdepth = (int)Math.Max(1, header.depths * Math.Pow(0.5, miplevel)) - 1;
		if(maxdepth > 1) {
		ImGui.SetNextItemWidth(200);
			changed |= ImGui.SliderInt("Depth", ref depth, 0, maxdepth);
			ImGui.SameLine();
		}
		...
		Aeth.Draw.AddText(pos + Vector2.One, 0xFF000000, header.format.ToString());
		Aeth.Draw.AddText(pos, 0xFFFFFFFF, header.format.ToString());
	}
	...
	private static unsafe extern FFI.Result GetPreview(File* tex, ushort width, ushort height, ushort miplevel, ushort depth);
