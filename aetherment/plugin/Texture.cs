// This is dumb
// TODO: figure out a way to get the device on rust side
...
public class TextureManager {
	...
	public CreateTextureDataDelegate createTextureData;
	public delegate IntPtr CreateTextureDataDelegate(TextureOptions options, FFI.Vec data);
	public unsafe IntPtr CreateTextureData(TextureOptions options, FFI.Vec data) {
		var desc = new Texture2DDescription {
			Width = options.width,
			Height = options.height,
			MipLevels = 1,
		...
		unsafe {
			var pitch = options.width * 4;
			var tex = new Texture2D(Aetherment.Device, desc, new SharpDX.DataRectangle(data.DataPtr, pitch));
			var resource = new ShaderResourceView(Aetherment.Device, tex, new ShaderResourceViewDescription {
				Format = desc.Format,
				Dimension = ShaderResourceViewDimension.Texture2D,
				Texture2D = {MipLevels = desc.MipLevels},
