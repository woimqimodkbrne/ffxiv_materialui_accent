...
pub struct Datas {
	...
	pub penumbra: Option<penumbra::Meta>,
}
...
impl Datas {
	pub fn tags(&self) -> HashMap<String, Vec<String>> {
		...
		if let Some(penumbra) = &self.penumbra {
			...
			penumbra.options.iter().for_each(|o| match o.deref() {
				MetaOption::Single(opt) | MetaOption::Multi(opt) => opt.options.iter().for_each(|o| o.files.iter().for_each(|(path, _)| TAGS.iter().for_each(|tag| {
					tag.regex.iter().for_each(|r| if r.is_match(&path) {
						tags.entry(tag.name.clone()).or_insert_with(|| Vec::new()).push(path.to_owned());
					})
				}))),
	...
	/// Cleanup creator mod, not enduser mod. everything is organized so poorly holy shit.
	pub fn cleanup(&self, mod_path: &PathBuf) {
		...
		if let Some(penumbra) = &self.penumbra {
			...
			penumbra.options.iter().for_each(|o| match o.deref() {
				MetaOption::Single(o) | MetaOption::Multi(o) => o.options.iter().for_each(|o| o.files.iter().for_each(|(_, f)| f.0.iter().for_each(|l| l.paths.iter().for_each(|p| {used_files.insert(p.replace("\\", "/"));})))),
				_ => {},
			})
		}
